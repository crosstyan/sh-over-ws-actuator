// Code borrow from
// https://github.com/zellij-org/zellij/blob/main/zellij-server/src/os_input_output.rs
...
use crate::{command::{RunCommand, TerminalAction}, error::{FatalError, LoggableError, ToAnyhow}};
...
fn handle_openpty(
    ...
    cmd: RunCommand,
    quit_cb: Box<dyn Fn(Option<i32>, RunCommand) + Send>, // u32 is the exit status
    terminal_id: u32,
) -> Result<(RawFd, RawFd)> {
    ...
    if command_exists(&cmd) {
        ...
        std::thread::spawn(move || {
            ...
            quit_cb(exit_status, cmd);
        });

        Ok((pid_primary, child_id as RawFd))
    } else {
        anyhow::bail!("Command '{}' does not exist", cmd.command.to_string_lossy())
    }
}
...
fn handle_terminal(
    ...
    quit_cb: Box<dyn Fn(Option<i32>, RunCommand) + Send>,
    terminal_id: u32,
) -> Result<(RawFd, RawFd)> {
...
/// If a [`TerminalAction::OpenFile(file)`] is given, the text editor specified
/// by environment variable `EDITOR` (or `VISUAL`, if `EDITOR` is not set) will
/// be started in the new terminal, with the given file open.
/// If [`TerminalAction::RunCommand(RunCommand)`] is given, the command will be
/// started in the new terminal.
...
fn spawn_terminal(
    ...
    orig_termios: termios::Termios,
    quit_cb: Box<dyn Fn(Option<i32>, RunCommand) + Send>, // u32 is the exit_status
...
impl ServerOsApi for ServerOsInputOutput {
    ...
    fn get_cwd(&self, pid: Pid) -> Option<PathBuf> {
        ...
        if let Some(process) = system_info.process(sysinfo::Pid::from(pid.as_raw() as usize)) {
            let cwd = process.cwd();
